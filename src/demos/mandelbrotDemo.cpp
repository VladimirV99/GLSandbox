...
MandelbrotDemo::MandelbrotDemo()
    : shader("../assets/mandelbrot.vert.glsl", "../assets/mandelbrot.frag.glsl")
{
}
...
static glm::vec4 find_ranges(std::vector<float>& data)
{
    ...
    glm::vec4 ranges = glm::vec4(data[lowest], data[lowest + length * 3 / 4 - 1],
                                 data[lowest + length * 7 / 8 - 1], data[size - 1]);
    return ranges;
}
...
void MandelbrotDemo::ProcessMouse(GLFWwindow* window, double xpos, double ypos, double xoffset,
                                  double yoffset)
{
}
...
bool MandelbrotDemo::DrawMenu() { return true; }
